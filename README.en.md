...
|---|---|---|---|
| `/research-ideation` | Topic assessment: judge whether a research idea is worth pursuing and whether it has been done | Works through the nine Heilmeier questions; searches the literature and sorts it into six categories (established findings, contested findings, null results, mechanisms, methodology, closest related work), verifying each paper at its source | An assessment — worth doing / needs narrowing / consider another direction — plus a set of adjacent topics and the PDFs in `papers/` |
