...
| 命令 | 用途 | 做法 | 产物 |
|---|---|---|---|
| `/research-ideation` | 科研选题：评估一个研究想法是否值得做、是否已有人做过 | 按 Heilmeier 九问梳理选题；检索文献并归入六类（已确立的发现、争议性发现、否定结论的发现、机制、方法论、最接近的相关研究），逐篇回源核验 | 评估报告 —— 值得做 / 需要收窄 / 建议换方向，附一批延伸选题 + `papers/` 里的 PDF |
