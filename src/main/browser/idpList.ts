...
export type IdpListResult = {
  ...
  skipped: SkippedIdp[];
  /**
   * 同名但 entityID 不同的机构名。**实测今天这份快照里是 0 条。**
   *
   * 先把实测说清楚，因为这里一度被说反过：2026-09-09 直连该端点跑 `parseIdpList`，
   * 1064 条里**重名 0 个**、本字段 0 条。名字不是唯一键这句话没错，但它今天成立的
   * 方向是**反的** —— 1064 个机构只有 938 个不同 entityID，
   * `https://passport.escience.cn/idp/shibboleth` 一个 ID 被 127 个中科院所共用。
   * 也就是说：今天下拉里不会有两行一样的文字，倒是有 127 行不同的文字指向同一个 ID
   *（那不是歧义，选哪一行登录都走同一套认证，见下）。
   *
   * 那还留着它做什么：**这是一份活的上游数据**，0 是今天的观测值不是协议保证。
   * 真出现同名不同 ID 时，下拉里就是两行一模一样的文字，用户选错一条会在 IdP 那边
   * 登录失败而界面无从解释 —— 解析层看得见这件事，报出来让调用方能消歧
   *（加后缀、并排显示 host 都行）。**别把它当成「今天需要一套消歧 UI」的依据。**
   *
