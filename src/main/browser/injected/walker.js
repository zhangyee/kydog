...
(() => {
  ...
  // 漏在外面的是「这个文档从头到尾没有一次快照见过它是 password」，三种走法：
  //
  ...
  //  c. 元素排在 **MAX_WALKED 之外**，遍历本身没有走到它 —— rememberIfPassword 压根
  //     没被调用过，跟采到没采到无关（那是 collect 的事）。这条依赖是本批把登记从
  //     collect() 挪进遍历时才新绑上的：挪之前登记只看「当次可见且采到」的元素，
  //     跟遍历上限无关；挪之后「登记」本身变成了遍历的子集，继承了它的上限。
  //     真实场景：页面前面有 12 万个元素、密码框排在最后 —— 第一次快照遍历在
  //     MAX_WALKED 处停手，密码框连走都没走到，world.pw 里没有它；随后站点把
  //     type 改成 text、前面内容被清掉 —— 下一次快照 value: "hunter2"、
  //     isPassword: false，明文进模型上下文与 transcript。
  //
  // 这三种只剩判据 3/4 兜底：写了 autocomplete=current-password 的不漏，
  ...
  //
  // **给第六批的验收标准**：登记不受任何遍历上限约束——常驻登记跑在 dom-ready /
  // MutationObserver 上，不经过 scan() 的 walked 计数，MAX_WALKED（以及未来任何
  // 新增的遍历/成本上限）都不许限制它能登记到的元素范围。上面的走法 c 就是「复用
  // walker 的遍历」这条捷径会把 MAX_WALKED 原样带过去的地方——按走法 a 写的用例
  // （只测「快照前改 type」）测不出这条依赖，必须单独覆盖。
  //
  ...
  // 有上界的是**它后面每个元素要做的事**，而 MAX_WALKED 是**唯一**还在的那道闸
  // （原来还有一道 MAX_EXAMINED 管强制 layout 的两个调用，本批已经拆掉：见
  // MAX_WALKED 的 docblock，examined ≤ walked 恒成立，单开一道是多余的天花板）。
  // 一次 C++ 侧的树遍历比几万次 getComputedStyle 便宜几个量级，这是刻意的取舍，
  // 不是漏了一道闸。
  const scan = (root) => {
