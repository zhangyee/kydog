...
> No need to repeat it where a table already has a title column. **Titles must be reproduced in full, no truncating with an ellipsis, no shortening of your own**
> — the reader will take them away to search with. Anything not verifiable against the full text is marked `[abstract only]`.
...
| Method | Representative study | What it actually measures | Known limits |
|---|---|---|---|
| | | | |
...
<M> metadata items disagreed with the original record and were corrected against the source (<name the most important one>).
...
**Verdict: <worth doing / needs narrowing first / switch direction>**
...
<recount: how far the closest studies got>

<appraise: whether the novelty they claim is a real gap or already filled; if partly filled, whether what remains can carry a project>

### Whether it can be pulled off

<recount: what each existing method costs, and who came unstuck at which step>

<appraise: whether the resources and timeline they hold buy the step they want>

### Whether the risk is where they think it is

<recount: which failure modes the contested findings and the negative findings point to>

<appraise: whether the risks they named in Q6 are the same set; whether any recur in the literature that they never mentioned>

### Whether the claimed value holds

<recount: how far the established findings support it>

<appraise: for the beneficiaries and the change they describe, how far the evidence supports it, and where extrapolation begins>
...
When the verdict is "needs narrowing":

- **The narrowed topic**: <one sentence, concrete enough to serve directly as a title>
- **Why it narrows that way**: <which part was cut and why it cannot hold up; why what remains can>
- **Things to settle before starting**: 3–6 items, each matching a specific failure mode in the literature, not generic research advice
...
>
> The good version:
> > González-Cabeza et al. (2025) report that three-lead reaches micro-F1 77% on static five-class
> > classification, close to 12-lead's 78%, while Zepeda-Echavarria et al. (2024) find sensitivity
> > for OMI on a real four-electrode device is only 65%. No study has yet attributed this gap — does
> > it come from lead selection, or from real acquisition conditions? On that basis one could …
>
> The bad version:
...
<review-style recounting + the topic drawn out of it + why it may be more worth doing than the original topic>
...
- Citation-chain tracking: starting from <seed paper>, <N> incoming
- Negative-results sweep: searched <keywords>, <result>
...
| Paper | Title | Source lookup | Metadata | Claim basis | Disposition |
|---|---|---|---|---|---|
| Author year, `DOI` | | | | | |
...
## Appendix B: The boundaries of this report

- Sources covered: <list>. Not covered: <list>, so <some category of literature> may have been missed.
- Claims marked `[abstract only]` were verified against the abstract only; the full text was not seen.
- Citation relations are available only from the `semantic` and `openalex` sources; every other "relatedness" judgment in this
  report is a proxy, and is labeled as such in the text. No impact factor data.
