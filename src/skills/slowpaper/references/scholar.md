...
**有一条候选解释，但它没有被复验过。** Chromium 的表单隐式提交发生在 `keypress`，
而 CDP 的 `dispatchKeyEvent` 只有 `keyDown` 且带 `text` 时才产生 char 事件；本项目的按键表
给 `Enter` 带上了 `text`（`actions.ts` 的 `KEYS`）——所以**那次否定观测有可能只是侦察工具
的问题**。但「这两个站点在本项目的 `key` 动作下真的会提交」**本项目一次都没量过**，
所以它仍然是一个开放问题，不是结论。

**在复验之前一律点提交按钮。** 点按钮对「提交控件不是 `type=submit`」的站点也成立，
是唯一一条实测走通过的路。Enter 那条路如果其实还是不通，`browser_act` 不会报错、
也不会等导航，紧接着的 `extract` 在首页 DOM 上跑，返回值一切正常而条数是 0 ——
页面没报错，你会以为搜过了。
...
**403 出现在检索提交之后。** Scholar 的提交控件是表单按钮，不属于普通 `<a href>`
链接的确定等待保证：导航事实若在本次输入期间到达，提交动作自己的返回值就会带
`HTTP 403`；若导航晚到，它会挂在**下一次**浏览器工具结果**头部**那行
`导航: [tab_…] 已打开 …，但服务器返回 HTTP 403`，**只报一次、报过就清**。
所以提交后先看本次返回；没有的话，下一次调用先看头部导航行。
...
**Scholar 的翻页是真正的当前 frame 链接。** `browser_act` 会在鼠标派发前保留它的
`<a href>` 导航意图，等主 frame 出现明确终态再返回；翻页点击自己的结果里就应同时看到
「已点击」与导航结论。仅为了等历史提交，不用再补一个 `wait`。

**但导航终态不等于结果列表已经稳定。** 这一批的收尾快照属于已经提交的新文档，仍可能
早于异步结果完成；看到内容没变绝不要再点一次：Scholar 对连续快速动作最敏感，多点的那一下
正是掉进 403 的形状。检索提交按钮不是普通链接，仍按上面 403 那段同时检查本次返回与
下一次结果头部的 `导航: [tab_…]`。

**怎么确认**：抽完之后拿这一页的第一条 `title` 与上一页的第一条比一眼 ——
**一模一样就是页面还没换**，别把它当成新的一页记下来，也别接着往下翻。
那正是「把第一页抽三遍、返回值一切正常」的样子。
（一个「点击前不成立、翻页后成立」且能证明异步列表稳定的条件本期还没有实测过，见「补齐程序」。）
