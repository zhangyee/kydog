...
——后者才是「整篇重写」的价码。只有加块、删块、换块顺序这种**章内的结构**变动
才需要重渲整章，而那也只是一章一百来行，不是整篇。

⚠️ **删一章 / 插一章 / 换章的顺序是另一件事，别按上一句估价。** 章号 `§N` 全篇连续、
`id === "c" + number`、目录条目、知识地图节点、页内锚点、图号、术语表的 `where`
全都跟着动。哪一处该怎么改，照 `references/layout.md`
「事后改结构：删一章 / 插一章 / 换顺序」一步步走——**5.2 不再确认提纲之后，
这条路是唯一的校准入口**，别临场发挥。
