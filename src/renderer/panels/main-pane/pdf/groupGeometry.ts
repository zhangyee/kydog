...
import { containsCenter, paddedRect, unionRect, type Rect } from './blockRect';
 ...
 * （并集再外扩 BLOCK_PAD）里，不许出现不属于它的行的中心点。
 *
...
export function checkGroupGeometry(groups: ParsedGroup[], lines: PageLine[]): void {
  ...
  for (const g of groups) {
    ...
    const rects = g.lines.map((n) => byId.get(n)).filter((l): l is PageLine => !!l).map(rectOf);
