...
export function SkillSyncModal({ status, onApply, onDismiss }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center">
      <div
        className="rounded-lg shadow-xl w-[640px] max-h-[80vh] overflow-auto p-6"
      ...
      >
        ...
        {status.pendingConflicts.map((c) => (
          <SkillBlock
            key={c.skill}
            c={c}
            selected={selected[c.skill] ?? new Set()}
            onToggle={(f) => toggle(c.skill, f)}
          />
        ))}
        ...
        <div className="flex justify-end gap-2 mt-6">
          <button
            onClick={onDismiss}
            disabled={busy}
            className="px-3 py-1 text-sm rounded hover:bg-[color:var(--color-hover-bg)] disabled:opacity-50"
          ...
          <button
            onClick={apply}
            disabled={busy}
            className="px-3 py-1 text-sm rounded hover:bg-[color:var(--color-accent-hover)] disabled:opacity-50"
...
function SkillBlock({
  ...
  return (
    <div
      ...
      style={{ borderWidth: 1, borderStyle: 'solid', borderColor: 'var(--color-ink-hair)' }}
    >
      <div className="font-medium" style={{ color: 'var(--color-ink)' }}>
        {c.skill}{' '}
        <span className="text-xs" style={{ color: 'var(--color-ink-faint)' }}>
          ({c.conflicts.length} 个文件冲突)
        </span>
      </div>
      <ul className="mt-1">
        {c.conflicts.map((f) => (
          <li key={f.relPath} className="text-sm">
            <label className="inline-flex items-center gap-2">
              ...
              />
              <span className="font-mono" style={{ color: 'var(--color-ink)' }}>{f.relPath}</span>
              <span className="text-xs" style={{ color: 'var(--color-ink-faint)' }}>
                disk {f.diskSha.slice(0, 7)} → ship {f.shippedSha.slice(0, 7)}
              </span>
            </label>
          </li>
        ))}
