...
export function PanelIcon({ side, filled = false, size = 14 }: Props) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 14 14" fill="none" style={{ display: 'block' }}>
      ...
      {side === 'left' && (
        <rect x="1.5" y="3" width={barW} height="8" fill={filled ? stroke : 'none'} opacity={filled ? 0.85 : 0.3} />
      )}
      {side === 'right' && (
        <rect x={14 - 1.5 - barW} y="3" width={barW} height="8" fill={filled ? stroke : 'none'} opacity={filled ? 0.85 : 0.3} />
      )}
    </svg>
  );
}
