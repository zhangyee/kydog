...
- **改了 `src/main/browser/injected/interact.js` 的 `measure` 会不会真的 `scrollIntoView`** → 回来重做 `e2e/61-browser.spec.ts` 那条「走真的 type…整批在碰页面之前就被挡下」。它靠「页面一个像素都没滚」区分密码框上两道**措辞逐字相同**的闸，measure 不真滚了就**静默变绿**。同一条里的非密码对照动作（断言 `scrollY > 0`）会先红，别把它当成多余的断言删掉。改夹具几何、`e2e/helpers.ts` 的窗口尺寸、`DEFAULT_VIEWPORT_HEIGHT` 同理。`npm test` 覆盖不到 e2e。
