...
- **改了 `src/main/browser/injected/interact.js` 的 `measure`、`e2e/61-browser.spec.ts` 里那个夹具（`bar` 的 `top`、密码框的 DOM 层级、那个撑高块）、`e2e/helpers.ts` 的窗口尺寸、或 `src/main/browser/browserService.ts` 的 `DEFAULT_VIEWPORT_HEIGHT`**（这四个是同一件事：那条 e2e 判据靠的几何与滚动行为）→ 回来重做 `e2e/61-browser.spec.ts` 那条「走真的 type…整批在碰页面之前就被挡下」。它靠「页面一个像素都没滚」区分密码框上两道**措辞逐字相同**的闸；`measure` 不真滚、或者 `scrollIntoView({block:'center'})` 落在那个密码框上时不再滚**窗口**（够不着、中间多了个滚动容器、页面整个滚不动、视口比页面还高，都算），判据就**静默变绿**。注意前提**不是**「密码框在首屏之外」（那是充分不必要条件：`scrollIntoView({block:'center'})` 对首屏内的元素照样滚）。同一条里有两条断言接着：注入之后**真滚一次再滚回来**的那条探针（`pwScrollsWindow`）、以及非密码对照动作（断言 `scrollY > 0`），别把它们当成多余的删掉。`npm test` 覆盖不到 e2e。
